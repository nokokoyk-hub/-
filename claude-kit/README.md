...
4. 終わるとき → `/wrapup` （ダイジェスト・北極星更新・学びの記録・引き継ぎメモまで完遂）

## 学びの記録システム（lessons.md）

各プロダクトリポジトリの `docs/lessons.md` に、ちゃぴの経験（❌事故・⭕成功・💛好み）を書き溜めて、
セッションをまたいで引き継ぐ仕組み。`/kickoff` で読み、`/wrapup` で追記・棚卸しする。

化石化（古いメモが足かせになる事故）を防ぐための決まりごと：

- **地図であって檻やない**：メモと現状が食い違うときは、従う前にのんに確認する
- **理由のない禁止は書かない**：日付＋理由（出典）つきの「原則」として書く
- **追記前に整合性チェック**：CLAUDE.md・ai_handoff.md・既存 lessons と矛盾しないか確認。矛盾はのんが裁定
- **外部情報は出典と日付つき**：鮮度の分からんメモは事故のもと
- **棚卸し**：古くなった項目は更新・削除。書くより捨てる方が大事
