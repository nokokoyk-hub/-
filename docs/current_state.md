...
3. 未完了：新しいクラウドタスクで、グローバルとプロジェクトの `AGENTS.md` 読み込み・関西弁の応対を実測
4. 必要時：他の開発リポ環境へ同じ導入コマンドを横展開
...
5. Codex共通ちゃぴ設定：main反映・設定リポ環境への登録済み。新しいクラウドタスクでの実測待ち

## 残タスク

1. 設定リポ環境から新しいCodexクラウドタスクを開始し、口調と指示元を実測確認
2. 他の開発リポでも共通ちゃぴ設定を使う場合、各リポ環境の Setup / Maintenance script へ同じ導入コマンドを登録
