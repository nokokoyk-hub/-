...
## NotionなどのMCP連携はクラウドでは使えない（2026-08-17時点）

- MCP連携（Notion・Obsidianなど）が効くのは**ローカルのCodex CLI・IDE拡張・デスクトップ**（`~/.codex/config.toml`）だけ
- **Codexクラウドには、環境設定からMCPを読み込む仕組みが現状ない**（公式ドキュメントにクラウド対応の記載なし。コミュニティでも要望のまま）
- さらにクラウドはエージェント作業中ネット遮断のため、仮に設定を持ち込めても api.notion.com へ届かない
- ローカルの設定ファイルはクラウドのコンテナへは引き継がれない。「ローカルでは読めるのにクラウドで読めない」のはこの仕様による

### 運用（構造で守る）

1. Codexクラウドの作業は、リポ内の `docs/current_state.md`（北極星）・handoff・worklog だけで完結させる
2. Notionの読み書きが必要な作業は、ローカルのCodex、またはNotion接続済みのClaude（クラウドのちゃぴ）が担当する
3. クラウドのCodexがNotionを読めなくても異常ではない。「読めていない」と明示して先へ進めばよい（共通設定の暴走ガードに準拠）
